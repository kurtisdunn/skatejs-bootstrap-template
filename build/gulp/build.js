...
module.exports = gulp.parallel(
  function () {
    return gulp.src('node_modules/jquery/dist/jquery.js').pipe(gulp.dest('dist')) && gulp.src('src/fonts/*.*').pipe(gulp.dest('dist/fonts')) &&  gulp.src('node_modules/bootstrap/dist/js/bootstrap.min.js').pipe(gulp.dest('dist'));
  },
  function () {
    ...
    return galv.trace('src/lib/index.js', {
      map: {
        skatejs: 'node_modules/skatejs/src/index.js'
      }
    }).createStream()
      // Scripts.
      .pipe(filterJs)
